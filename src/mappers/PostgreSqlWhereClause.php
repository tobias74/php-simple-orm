...
class PostgreSqlWhereClause2013 extends \VisitableSpecification\AbstractCriteriaVisitor
{
  //
  ...
  {
    $column = $this->context->getResponsibleMapperForField($criteria->getField())->getPreparedColumnForField($criteria->getField());
    $whereClause = " ( ".$column." BETWEEN '" .addslashes($criteria->getStartValue())."' AND '".addslashes($criteria->getEndValue())."' ) ";
    $this->setClauseForCriteria($criteria, $whereClause);
  ...
  {
    $column = $this->context->getResponsibleMapperForField($criteria->getGeometryField())->getPreparedColumnForField($criteria->getGeometryField());

    $whereClause = " ST_DWithin(".$column.", ST_MakePoint(".$criteria->getLongitude().",".$criteria->getLatitude().")::geography,".$criteria->getMaximumDistance().") ";
